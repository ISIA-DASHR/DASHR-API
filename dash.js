  ...
  { 
	   ...
	   if(this.video.event_handlers.onMetaData)
	  {
		  ...
		  for (var i= 0; i< this.video.mpd.map.periods[this.currentPeriod].asets.length; i++)
		  {
			  ...
			  tracklist.push({auto : true, lang: track.lang, type : track.type, id_aset : track.id_aset, selected : (this.currentTracks[track.type].id_aset == track.id_aset && this.currentTracks[track.type].autoswitch!==false) });
